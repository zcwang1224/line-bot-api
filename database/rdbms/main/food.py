...
class Food(Base, BaseUtil):
    ...
    food_id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    f_name = Column(String(100), nullable=False, comment="店家名")
    b_hours = Column(String(120), comment="營業時間")
    ...
    gmaps_url = Column(Text, comment="GOOGLE MAPS連結")
    f_type = Column(String(3), comment="類型")
    comm = Column(Integer, comment="評論數")
    area = Column(String(3), comment="區域")
    ...
    def __repr__(self):
        return "<%s(id=%s, name=%s)>" % (self.__class__.__name__, self.food_id, self.f_name)
